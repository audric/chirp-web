...
Web-based radio memory file converter powered by [CHIRP](https://chirp.danplanet.com). Upload a radio memory image, pick a target radio, and download the converted file — no desktop install needed.
...
│   ├── converter.py         # CHIRP integration (detect, convert, save)
│   ├── routers/
│   │   ├── api.py           # REST API (radios, detect, convert, download)
...
| POST   | `/api/convert`        | Convert uploaded file to target radio    |
